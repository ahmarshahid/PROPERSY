...
export default function N8nBidConsole({ defaultWebhookUrl = "/api/n8n-proxy" }) {
  const webhookUrl = defaultWebhookUrl; // fixed as proxy
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [responses, setResponses] = useState<Array<{ id: number; role: string; content: string; parsed?: any }>>([]);
  const [activeTab, setActiveTab] = useState<"pretty" | "stream" | "raw" | "logs">("pretty");

  // Added: logs state
  const [showLogs, setShowLogs] = useState(true);
  const [netLog, setNetLog] = useState<string>("");      // network timeline
  const [liveLog, setLiveLog] = useState<string>("");    // live response text (chunked)

  const t0Ref = useRef<number>(0);
  const companyRef = useRef<HTMLInputElement | null>(null);
  const rfpRef = useRef<HTMLInputElement | null>(null);
  const promptRef = useRef<HTMLInputElement | null>(null);

  const logNet = (line: string) => {
    setNetLog(prev => prev + (prev ? "\n" : "") + line);
  };

  function tryParseJSON(text: string) {
    try { const obj = JSON.parse(text); return obj && typeof obj === "object" ? obj : null; } catch { return null; }
  }

  function normaliseChatPayload(result: any) {
    const out: Array<{ role: string; content: string; parsed?: any }> = [];
    const pushMsg = (content: any) => {
      if (content == null) return;
      if (typeof content === "object") {
        if (content.message || content.Message) {
          out.push({ role: "assistant", content: String(content.message ?? content.Message), parsed: content });
          ...
          out.push({ role: "assistant", content: typeof body === "string" ? body : JSON.stringify(body, null, 2), parsed: parsed ?? content });
        } else {
    ...
    const flattenAny = (x: any): void => {
      if (Array.isArray(x)) x.forEach(flattenAny);
      else if (x && typeof x === "object") {
        if ("message" in x || "Message" in x || "text" in x || "output" in x) pushMsg(x);
        else if ("data" in x) flattenAny((x as any).data);
        else if ("items" in x) flattenAny((x as any).items);
        else if ("messages" in x) flattenAny((x as any).messages);
        else pushMsg(x);
      } else pushMsg(x);
    };
    flattenAny(result);
  ...
  const writers = useMemo(() => {
    const buckets: Record<string, any[]> = { executive: [], technical: [], pricing: [], references: [], timeline: [], other: [] };
    for (const r of responses) {
...
export default function N8nBidConsole({ defaultWebhookUrl = "/api/n8n-proxy" }) {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <header className="mb-6">
        <h1 className="text-2xl font-bold">Propersy Bid Console</h1>
      ...
      <form onSubmit={runWorkflow} className="space-y-4 bg-white/5 p-4 rounded-2xl border border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="block"><span className="text-sm">Company Profile PDF</span><input ref={companyRef} type="file" accept="application/pdf" className="mt-1 block w-full"/></label>
          <label className="block"><span className="text-sm">RFP PDF</span><input ref={rfpRef} type="file" accept="application/pdf" className="mt-1 block w-full"/></label>
        </div>

        <label className="block"><span className="text-sm">Message (optional)</span>
        ...
        <section className="mt-6 border border-white/10 rounded-2xl p-4">
          <div className="flex gap-2 mb-4 text-sm">
            {[["pretty","Writers view"],["stream","All messages"],["raw","Raw"],["logs","Logs"]].map(([k,label]) => (
              <button key={k} onClick={(e)=>{e.preventDefault();setActiveTab(k as any);}} className={`px-3 py-1 rounded-lg border ${activeTab===k?"bg-white/10 border-white/30":"border-white/10"}`}>{label}</button>
            ))}
          </div>
              ...
              {Object.entries(writers).map(([bucket, items]) => (
                <div key={bucket} className="border border-white/10 rounded-xl p-3">
                  ...
                  {items.length===0 ? <p className="text-sm opacity-60">No content</p> : (
                    <ul className="space-y-2 text-sm">{items.map((r:any)=>(
          ...
          {activeTab==="raw" && (
            <pre className="text-xs bg-black/60 rounded-xl p-3 overflow-auto max-h-96">{JSON.stringify(responses.map(r=> (r as any).parsed ?? r.content), null, 2)}</pre>
          )}
...
}-2xl font-bold">Propersy Bid Console</h1>
        <p className="text-sm opacity-80">
          Upload two PDFs and watch live logs while the workflow runs.
        </p>
      </header>
